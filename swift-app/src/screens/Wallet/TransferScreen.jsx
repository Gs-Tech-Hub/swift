...
export default function TransferScreen({ navigation }) {
  ...
  const handleConfirm = async () => {
    if (pin.length === 4 && accountNumber && amount) {
      ...
      try {
        await transfer(Number(amount), accountNumber, narration || "Transfer");
        setSuccess(true);
      } catch (err) {
        if (err.message === "INSUFFICIENT") {
          Alert.alert("Insufficient Balance", "Please deposit funds to continue", [{ text: "Deposit", onPress: () => navigation.navigate("DepositScreen") }]);
        } else {
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      {success ? (
        <View style={styles.centered}>
          ...
          <Text style={[styles.successText, { color: theme.primary }]}>Transfer Successful</Text>
          <Text style={{ color: theme.textMuted, marginBottom: 20 }}>₦{amount} sent to {accountNumber}</Text>
        ...
        <>
          ...
          <TextInput placeholder="1234567890" placeholderTextColor={theme.textMuted} keyboardType="numeric" maxLength={10} value={accountNumber} onChangeText={setAccountNumber} style={[styles.input, { borderColor: theme.border, color: theme.primary }]} />
          ...
          <TextInput placeholder="What's this for?" placeholderTextColor={theme.textMuted} value={narration} onChangeText={setNarration} style={[styles.input, { borderColor: theme.border, color: theme.primary }]} />
      ...
      <Modal visible={showPinModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: theme.background }]}>
            ...
            <TouchableOpacity onPress={() => setShowPinModal(false)}><Text style={{ color: theme.danger, marginTop: 10 }}>Cancel</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1, borderRadius: 10, padding: 12, fontSize: 16, marginBottom: 20, width: "100%" },
